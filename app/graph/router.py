...
def route_decision(state: GraphState) -> dict:
    ...
    if score >= HALLUCINATION_THRESHOLD:
        ...
        decision = "retry"
        retry_count += 1
        summary = f"Score {score} < threshold {HALLUCINATION_THRESHOLD} - retrying (attempt {retry_count} / {max_retries})"
    else:
        decision = "stop"
        summary = f"Score {score} < threshold {HALLUCINATION_THRESHOLD} - max retries reached, stopping"
